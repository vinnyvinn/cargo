        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="form-group">
                    <select name="service" required id="service" onchange="checkType(this)" data-placeholder="Search and select Service" class="form-control select2">
                        <option value="">Search and select service</option>
                        @foreach($services as $service)
                            <option value="{{$service}}">{{ ucwords($service->name) }} ~  BP KES {{ number_format($service->fixed == 0 ? $service->rate : ($service->rate * $exrate), 2) }} | USD {{ number_format( $service->fixed == 0 ? ($service->rate / $exrate) : $service->rate, 2) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="col-sm-3">
                <div class="form-group">
                    <input type="number" required id="service_units" name="service_units" placeholder="Value" class="form-control">
                </div>
            </div>
            <div class="col-sm-3">
                <div class="form-group">
